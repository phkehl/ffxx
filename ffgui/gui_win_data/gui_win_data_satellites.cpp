...
GuiWinDataSatellites::GuiWinDataSatellites(const std::string& name, const InputPtr& input) /* clang-format off */ :
    GuiWinData(name, { 80, 40, 0, 0 }, ImGuiWindowFlags_None, input),
...
void GuiWinDataSatellites::_ProcessData(const InputDataPtr& data)
{
...
void GuiWinDataSatellites::_DrawContent()
{
    ...
    if (tabbarMode_.Begin()) {
        doSky_ = tabbarMode_.Item("Sky").isSelected_;
        doList_ = tabbarMode_.Item("List").isSelected_;
        tabbarMode_.End();
    }
    ...
    if (tabbarFilter_.Begin()) {
        for (auto& f : FILTERS) {
            if (tabbarFilter_.Item(doList_ ? counts_[f.ix].labelList_ : counts_[f.ix].labelSky_).newlySelected_) {
                filterIx_ = f.ix;
                _UpdateSatellites();
            }
        }
    ...
    if (epoch_) {
        ...
        for (auto& sat : epoch_->epoch_.sats) {
            ...
            table_.AddCellTextF("%-5s %c%c%c%c", sat.orbUsedStr,
                (sat.orbAvail & SatOrb::ALM) == SatOrb::ALM ? 'A' : '-',
                (sat.orbAvail & SatOrb::EPH) == SatOrb::EPH ? 'E' : '-',
                (sat.orbAvail & SatOrb::PRED) == SatOrb::PRED ? 'P' : '-',
                (sat.orbAvail & SatOrb::OTHER) == SatOrb::OTHER ? 'O' : '-');
