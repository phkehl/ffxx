...
void GuiWinDataConfig::_ProcessData(const InputDataPtr& data)
{
...
void GuiWinDataConfig::_DrawToolbar()
{
    ...
    if (ImGui::BeginPopup("View")) {
        Gui::SeparatorTitle("Show items for:");
...
void GuiWinDataConfig::_DrawContent()
{
    ...
    if (tabbarMain_.Begin()) {
        if (tabbarMain_.Item(TB_DATABASE).isSelected_) {
            if (itemsEdit_.empty()) {
                _DrawItems();
            } else {
        ...
        if (tabbarMain_.Item(TB_LOG).isSelected_) {
            log_.DrawWidget();
        }
        tabbarMain_.End();
    }
    ...
    if (/*drawChild &&*/ ImGui::BeginTable("List", 10, flags, ImGui::GetContentRegionAvail())) {
        ...
        while (clipper.Step()) {
            ...
            for (int row = clipper.DisplayStart; row < clipper.DisplayEnd; row++)
            {
                ...
                if (item.anyNonDefault_) { ImGui::PushStyleColor(ImGuiCol_Text, C_TEXT_HIGHLIGHT_FG()); }
                if (item.numEdited_ > 0) { ImGui::TableSetBgColor(ImGuiTableBgTarget_CellBg, C_TEXT_HIGHLIGHT_BG()); }
                ...
                if (item.anyNonDefault_) { ImGui::PopStyleColor(); }
...
void GuiWinDataConfig::_DrawEditorTabs()
{
    ...
    for (auto item : itemsEdit_) {
        const auto tb = tabbarEdit_.Item(
            item->db_.name_, nullptr, true, item->numEdited_ > 0 ? ImGuiTabItemFlags_UnsavedDocument : ImGuiTabItemFlags_None);
        if (tb.isSelected_) {
            editItem = item;
        }
        if (tb.gotClosed_) {
...
void GuiWinDataConfig::_PollData(const InputDataMsg& msg)
{
    if (msg.origin_ != InputDataMsg::RCVD) {
        return;
    }
...
void GuiWinDataConfig::_SaveData(const InputDataMsg& msg)
{
    if (msg.origin_ != InputDataMsg::RCVD) {
        return;
    }
...
void GuiWinDataConfig::Item::Sync()
{
    ...
    for (int ix = Item::FLASH; ix <= Item::RAM; ix++) {
        ...
        if (currNonDefault_[ix] || editNonDefault_[ix]) {
            anyNonDefault_ = true;
        }
