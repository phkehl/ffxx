...
GuiWinDataLog::GuiWinDataLog(const std::string& name, const InputPtr& input) /* clang-format off */ :
    GuiWinData(name, { 120, 35, 0, 0 }, ImGuiWindowFlags_None, input),
...
void GuiWinDataLog::_ProcessData(const InputDataPtr& data)
{
    switch (data->type_) {
        case InputDataType::MSG: {
            ...
            switch (msg.msg_.proto_) {  // clang-format off
                ...
                case Protocol::QGC:    log_.AddLine(tmp, C_LOG_MSG_QGC(),    (msg.time_ - GUI_VAR.t0).Stringify() ); break;
...
{
    if (ImGui::BeginTable("Counts", 14, ImGuiTableFlags_HighlightHoveredColumn)) {  // clang-format off
        ...
        ImGui::TableSetupColumn("     QGC", ImGuiTableColumnFlags_WidthFixed | (stats_.n_qgc_    > 0 ? ImGuiTableColumnFlags_None : ImGuiTableColumnFlags_Disabled));
        ...
        ImGui::TableNextColumn(); if (stats_.n_qgc_    > 0) { ImGui::PushStyleColor(ImGuiCol_Text, C_LOG_MSG_QGC());    ImGui::Text("%8" PRIuMAX, stats_.n_qgc_);    ImGui::PopStyleColor(); }
        ...
        ImGui::TableNextColumn(); if (stats_.n_qgc_    > 0) { ImGui::PushStyleColor(ImGuiCol_Text, C_LOG_MSG_QGC());    ImGui::Text("%8" PRIuMAX, stats_.s_qgc_);    ImGui::PopStyleColor(); }
