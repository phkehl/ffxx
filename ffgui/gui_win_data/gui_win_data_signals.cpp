...
void GuiWinDataSignals::_ProcessData(const InputDataPtr& data)
{
...
void GuiWinDataSignals::_DrawContent()
{
    ...
    if (tabbarFilter_.Begin()) {
        for (auto& f : FILTERS) {
            if (tabbarFilter_.Item(counts_[f.ix].label_).newlySelected_) {
                filterIx_ = f.ix;
                _UpdateSignals();
            }
        }
        ...
        if (dxy2 < r2) {
            ...
            if (binIx < cnoSky_.bins_.size()) {
                ...
                if (bin.count_[filterIx_] > 0) {
                    ...
                    draw->PathArcTo(canvasCent, radiusPx * bin.r0_, bin.a0_, bin.a1_);
                    draw->PathArcTo(canvasCent, radiusPx * bin.r1_, bin.a1_, bin.a0_);
                    draw->PathStroke(C_C_GREY(), 3.0f, ImDrawFlags_Closed);
                }
            }
        }
    }
...
void GuiWinDataSignals::_UpdateSignals()
{
    ...
    if (epoch_) {
        ...
        for (auto& sig : epoch_->epoch_.sigs) {
            ...
            table_.AddCellTextF("%s %s %s", sig.prUsed ? "PR" : "--", sig.crUsed ? "CR" : "--", sig.doUsed ? "DO" : "--");
            ...
            if (sig.prUsed /*sig->prRes != 0.0*/) {
                ...
                table_.SetCellSort(std::fabs(prResSort));
            } else {
