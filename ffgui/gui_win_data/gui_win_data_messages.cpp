...
void GuiWinDataMessages::_ProcessData(const InputDataPtr& data)
{
    switch (data->type_) {
        case InputDataType::MSG: {
        ...
        case InputDataType::EPOCH:
        case InputDataType::DEBUG:
        case InputDataType::EVENT:
            break;
    }
}
...
void GuiWinDataMessages::_DrawToolbar()
{
    ...
    if (ImGui::Button(ICON_FK_ERASER "##ClearMsg", GUI_VAR.iconSize)) {
        ...
        if (displayedEntry_->second.guiMsg_) {
            displayedEntry_->second.guiMsg_->Clear();
        }
    }
    ImGui::EndDisabled();
...
void GuiWinDataMessages::_DrawMessage()
{
    ...
    const ImVec2 msgInfoSize   { 0, (3 * GUI_VAR.charSizeY) + (2 * GUI_VAR.imguiStyle->ItemSpacing.y) };
    ...
    if (ImGui::BeginChild("##MsgInfo", msgInfoSize)) {
        ...
        if (receiverReady) {
            ImGui::Text("%-20s %c %-8s %-8s %5" PRIuMAX " %5" PRIuMAX " %5.1f %5.1f %5" PRIuMAX "",
                ...
                InputDataMsg::OriginStr(msg.origin_), msg.msg_.seq_, info.count_, info.age_ != 0.0f ? info.age_ : NAN,
                info.msgRate_ != 0.0f ? info.msgRate_ : NAN, msg.msg_.Size());
        } else {
            ImGui::Text("%-20s %c %-8s %-8s %5" PRIuMAX " %5" PRIuMAX "     -     - %5" PRIuMAX "",
                msg.msg_.name_.c_str(), info.recent_ ? '*' : ' ', ProtocolStr(msg.msg_.proto_),
                InputDataMsg::OriginStr(msg.origin_), msg.msg_.seq_, info.count_, msg.msg_.Size());
        }
        Gui::TextDim("Info:");
        ImGui::SameLine();
        ImGui::TextWrapped("%s", msg.msg_.info_.empty() ? "-" : msg.msg_.info_.c_str());
    }
    ImGui::EndChild();
    ...
    // Message details
    ImGui::PushID(msg.msg_.name_.c_str());
    if (ImGui::BeginChild(
            "##MsgDetails", msgDetailSize, false, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoScrollWithMouse)) {

        bool doDetails = false;
        bool doJson = !info.guiMsg_;

        if (info.guiMsg_ && info.tabbar_.Begin()) {
            doDetails = info.tabbar_.Item("Details").isSelected_;
            doJson = info.tabbar_.Item("JSON").isSelected_;
            info.tabbar_.End();
        }
        ...
        if (doJson) {
            ...
            info.jsonTextEditor_.Render("##GuiMsgJson");

            // FIXME: why isn't this blocking interaction with the editor?
            // ImGui::SetCursorScreenPos(offs);
            ...
            if (ImGui::BeginPopupContextItem("Copy")) {
                if (ImGui::MenuItem("Copy JSON")) {
                    ImGui::SetClipboardText(info.json_.c_str());
                }
                ImGui::EndPopup();
            }
        }
    ...
    guiMsg_    { std::move(guiMsg) }  // clang-format on
{
    ...
    jsonTextEditor_.SetLanguage(TextEditor::Language::Json());
    jsonTextEditor_.SetShowSpacesEnabled(false);
    jsonTextEditor_.SetReadOnlyEnabled(true);
    jsonTextEditor_.SetShowLineNumbersEnabled(false);
    jsonTextEditor_.SetLineFoldingEnabled(false);
...
void GuiWinDataMessages::MsgInfo::Update(const InputDataMsgPtr& msg)
{
    if (!msg) {
        return;
    }
    ...
    json_ = ParserMsgToJson(msg_->msg_).dump(4);
    jsonTextEditor_.SetText(json_);
}
