...
GuiWinInputCorr::GuiWinInputCorr(const std::string& name) /* clang-format off */ :
    GuiWinInput(name, { 95, 25, 0, 0 }, ImGuiWindowFlags_None, Input::Create(name, InputType_Corr)),
...
void GuiWinInputCorr::_ProcessData(const InputDataPtr& data)
{
    if (data->type_ == InputDataType::MSG) {
        auto& msg = DataPtrToDataMsg(data);
        status_.Update(msg);

        // Forward data
        for (auto& [name, input] : Input::GetAll()) {
            if (!input->receiver_) {
                continue;
            }
...
{
    if (ImGui::BeginTable("Status", 9, ImGuiTableFlags_Borders | ImGuiTableFlags_NoHostExtendX)) {
        ImGui::TableSetupColumn("##empty", ImGuiTableColumnFlags_WidthFixed);
        ImGui::TableSetupColumn("Station", ImGuiTableColumnFlags_WidthFixed);
        ImGui::TableSetupColumn("GPS MSM", ImGuiTableColumnFlags_WidthFixed);
        ImGui::TableSetupColumn("GAL MSM", ImGuiTableColumnFlags_WidthFixed);
        ImGui::TableSetupColumn("BDS MSM", ImGuiTableColumnFlags_WidthFixed);
        ImGui::TableSetupColumn("GLO MSM", ImGuiTableColumnFlags_WidthFixed);
        ImGui::TableSetupColumn("GLO bias", ImGuiTableColumnFlags_WidthFixed);
        ImGui::TableSetupColumn("Noise", ImGuiTableColumnFlags_WidthFixed);
        ImGui::TableSetupColumn("Latency", ImGuiTableColumnFlags_WidthFixed);
        ImGui::TableHeadersRow();

        ImGui::TableNextRow();
        ImGui::TableNextColumn(); ImGui::Selectable("Status", false, ImGuiSelectableFlags_SpanAllColumns);
        ImGui::TableNextColumn(); ImGui::TextColored(status_.okSta_     ? C4_C_BRIGHTGREEN() : C4_C_BRIGHTRED(), status_.okSta_     ? "   :-)" : "   :-(");
        ImGui::TableNextColumn(); ImGui::TextColored(status_.okGps_     ? C4_C_BRIGHTGREEN() : C4_C_BRIGHTRED(), status_.okGps_     ? "   :-)" : "   :-(");
        ImGui::TableNextColumn(); ImGui::TextColored(status_.okGal_     ? C4_C_BRIGHTGREEN() : C4_C_BRIGHTRED(), status_.okGal_     ? "   :-)" : "   :-(");
        ImGui::TableNextColumn(); ImGui::TextColored(status_.okBds_     ? C4_C_BRIGHTGREEN() : C4_C_BRIGHTRED(), status_.okBds_     ? "   :-)" : "   :-(");
        ImGui::TableNextColumn(); ImGui::TextColored(status_.okGlo_     ? C4_C_BRIGHTGREEN() : C4_C_BRIGHTRED(), status_.okGlo_     ? "   :-)" : "   :-(");
        ImGui::TableNextColumn(); ImGui::TextColored(status_.okBias_    ? C4_C_BRIGHTGREEN() : C4_C_BRIGHTRED(), status_.okBias_    ? "   :-)" : "   :-(");
        ImGui::TableNextColumn(); ImGui::TextColored(status_.okNoise_   ? C4_C_BRIGHTGREEN() : C4_C_BRIGHTRED(), status_.okNoise_   ? "   :-)" : "   :-(");
        ImGui::TableNextColumn(); ImGui::TextColored(status_.latencyOk_ ? C4_C_BRIGHTGREEN() : C4_C_BRIGHTRED(), status_.latencyOk_ ? "   :-)" : "   :-(");

        ImGui::TableNextRow();
        ImGui::TableNextColumn(); ImGui::Selectable("Count", false, ImGuiSelectableFlags_SpanAllColumns);
        ImGui::TableNextColumn(); ImGui::Text("%9" PRIuMAX, status_.nSta_);
        ImGui::TableNextColumn(); ImGui::Text("%9" PRIuMAX, status_.nGps_);
        ImGui::TableNextColumn(); ImGui::Text("%9" PRIuMAX, status_.nGal_);
        ImGui::TableNextColumn(); ImGui::Text("%9" PRIuMAX, status_.nBds_);
        ImGui::TableNextColumn(); ImGui::Text("%9" PRIuMAX, status_.nGlo_);
        ImGui::TableNextColumn(); ImGui::Text("%9" PRIuMAX, status_.nBias_);
        ImGui::TableNextColumn(); ImGui::Text("%9" PRIuMAX, status_.nNoise_);
        ImGui::TableNextColumn(); ImGui::Text("%9.3f", status_.latency_);

        ImGui::TableNextRow();
        ImGui::TableNextColumn(); ImGui::Selectable("Signals tot", false, ImGuiSelectableFlags_SpanAllColumns);
        ImGui::TableNextColumn();
        ImGui::TableNextColumn(); if (latestEpoch_) { ImGui::Text("%6d", latestEpoch_->epoch_.numSigUsed.numGps); }
        ImGui::TableNextColumn(); if (latestEpoch_) { ImGui::Text("%6d", latestEpoch_->epoch_.numSigUsed.numGal); }
        ImGui::TableNextColumn(); if (latestEpoch_) { ImGui::Text("%6d", latestEpoch_->epoch_.numSigUsed.numBds); }
        ImGui::TableNextColumn(); if (latestEpoch_) { ImGui::Text("%6d", latestEpoch_->epoch_.numSigUsed.numGlo); }

        if (latestEpoch_) {
            const auto& n = latestEpoch_->epoch_.numSigUsedL1;
            if ((n.numGps + n.numGal + n.numBds + n.numGlo) > 0) {
                ImGui::TableNextRow();
                ImGui::TableNextColumn(); ImGui::Selectable("Signals L1", false, ImGuiSelectableFlags_SpanAllColumns);
                ImGui::TableNextColumn();
                ImGui::TableNextColumn(); if (latestEpoch_) { ImGui::Text("%6d", n.numGps); }
                ImGui::TableNextColumn(); if (latestEpoch_) { ImGui::Text("%6d", n.numGal); }
                ImGui::TableNextColumn(); if (latestEpoch_) { ImGui::Text("%6d", n.numBds); }
                ImGui::TableNextColumn(); if (latestEpoch_) { ImGui::Text("%6d", n.numGlo); }
            }
        }

        if (latestEpoch_) {
            const auto& n = latestEpoch_->epoch_.numSigUsedE6;
            if ((n.numGps + n.numGal + n.numBds + n.numGlo) > 0) {
                ImGui::TableNextRow();
                ImGui::TableNextColumn(); ImGui::Selectable("Signals E6", false, ImGuiSelectableFlags_SpanAllColumns);
                ImGui::TableNextColumn();
                ImGui::TableNextColumn(); if (latestEpoch_) { ImGui::Text("%6d", n.numGps); }
                ImGui::TableNextColumn(); if (latestEpoch_) { ImGui::Text("%6d", n.numGal); }
                ImGui::TableNextColumn(); if (latestEpoch_) { ImGui::Text("%6d", n.numBds); }
        ...
        if (latestEpoch_) {
            const auto& n = latestEpoch_->epoch_.numSigUsedL2;
            if ((n.numGps + n.numGal + n.numBds + n.numGlo) > 0) {
                ImGui::TableNextRow();
                ImGui::TableNextColumn(); ImGui::Selectable("Signals L2", false, ImGuiSelectableFlags_SpanAllColumns);
                ImGui::TableNextColumn();
                ImGui::TableNextColumn(); if (latestEpoch_) { ImGui::Text("%6d", n.numGps); }
                ImGui::TableNextColumn(); if (latestEpoch_) { ImGui::Text("%6d", n.numGal); }
                ImGui::TableNextColumn(); if (latestEpoch_) { ImGui::Text("%6d", n.numBds); }
        ...
        if (latestEpoch_) {
            const auto& n = latestEpoch_->epoch_.numSigUsedL5;
            if ((n.numGps + n.numGal + n.numBds + n.numGlo) > 0) {
                ImGui::TableNextRow();
                ImGui::TableNextColumn(); ImGui::Selectable("Signals L5", false, ImGuiSelectableFlags_SpanAllColumns);
                ImGui::TableNextColumn();
                ImGui::TableNextColumn(); if (latestEpoch_) { ImGui::Text("%6d", n.numGps); }
                ImGui::TableNextColumn(); if (latestEpoch_) { ImGui::Text("%6d", n.numGal); }
                ImGui::TableNextColumn(); if (latestEpoch_) { ImGui::Text("%6d", n.numBds); }
...
void GuiWinInputCorr::Status::Update(const InputDataMsg& msg)
{
