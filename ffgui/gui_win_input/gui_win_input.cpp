    ...
    log_         { WinName(), dummyEntries, false, 2500 }  // clang-format on
{
    ...
    input_->AddObserver(this, std::bind(&GuiWinInput::_ProcessInputDataMain, this, std::placeholders::_1));
...
void GuiWinInput::_ProcessData(const InputDataPtr& /*data*/) {}
...
void GuiWinInput::_ProcessInputDataMain(const InputDataPtr& data)
{
    ...
    // Process data in derived class (Receiver, Logfile, ...), and data windows
    _ProcessData(data);
}
...
#define DATA_WINS_DEFS(_P_) /* class   flags                                              name/title          tooltip/menu         button */ \
    ...
    _P_(GuiWinDataSignals,     InputType_Receiver | InputType_Logfile | InputType_Corr,  "Signals",           "Signals",           ICON_FK_SIGNAL         "##Signals") \
...
void GuiWinInput::DrawWindow()
{
    ...
    if (ImGui::BeginPopup("Options")) {
        ...
        if (ImGui::Selectable("Summon data windows")) {
            summonDataWindows = true;
        }
        if (ImGui::Selectable("Close data windows")) {
            closeDataWindows = true;
        }
...
#undef _EVAL1
    ...
    if (input_->database_) {
        ...
        if (ImGui::BeginPopup("DbSettings")) {
            Gui::SeparatorTitle("Database status");
            ...
            ImGui::Separator();
            Gui::SeparatorTitle("Database settings");
