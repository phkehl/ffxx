...
GuiWinInputLogfile::GuiWinInputLogfile(const std::string &name) /* clang-format off */ :
    GuiWinInput(name, { 90, 25, 0, 0 }, ImGuiWindowFlags_None, Input::Create(name, InputType_Logfile)),
    ...
    history_      { WinName(), GUI_CFG.logfileHistory, 30 }  // clang-format on
{
    ...
    input_->logfile_->SetSpeed(cfg_.noLimit ? Logfile::SpeedMode::NOLIMIT : cfg_.speedMode, cfg_.speedValue);
};
...
void GuiWinInputLogfile::_ProcessData(const InputDataPtr& /*data*/)
{
}
...
void GuiWinInputLogfile::_DrawControls()
{
    ...
    // Play speed
    {
        // Limit/no limit checkbox
        if (ImGui::Checkbox("##SpeedLimit", &cfg_.noLimit)) {
            logfile.SetSpeed(cfg_.noLimit ? Logfile::SpeedMode::NOLIMIT : cfg_.speedMode, cfg_.speedValue);
        }
        Gui::ItemTooltip("Limit play speed");

        ImGui::SameLine(0, 0);

        ImGui::BeginDisabled(cfg_.noLimit);

        // Speed value input
        ImGui::SetNextItemWidth(GUI_VAR.charSizeX * 8.0f);
        const char* fmt = (cfg_.noLimit ? "no limit" : "%.1f");
        if (ImGui::DragFloat("##Speed", &cfg_.speedValue, 0.1f, Logfile::SPEED_MIN, Logfile::SPEED_MAX, fmt, ImGuiSliderFlags_AlwaysClamp)) {
            cfg_.noLimit = false;
            logfile.SetSpeed(cfg_.speedMode, cfg_.speedValue);
        }
        Gui::ItemTooltip(cfg_.speedMode == Logfile::SpeedMode::MSG ? "Play speed [messages/s]" : "Play speed [epochs/s]");

        ImGui::SameLine(0, 0);

        // Speed presets
        std::array<const char*, 11> presets = { { "1.0", "2.0", "5.0", "10.0", "20.0", "50.0", "100.0", "500.0", "1000.0", "2000.0", "5000.0" } };
        if (ImGui::BeginCombo("##SpeedChoice", NULL, ImGuiComboFlags_HeightLarge | ImGuiComboFlags_NoPreview)) {
            for (std::size_t ix = 0; ix < presets.size(); ix++) {
                if (ImGui::Selectable(presets[ix])) {
                    StrToValue(presets[ix], cfg_.speedValue);
                    logfile.SetSpeed(cfg_.speedMode, cfg_.speedValue);
                }
            }
            ImGui::EndCombo();
        ...
        // Speed mode
        std::array<const char*, 2> modes = { { "epochs/s", "msgs/s" } };
        ImGui::SetNextItemWidth(GUI_VAR.charSizeX * 12.0f);
        if (ImGui::BeginCombo("##ModeChoice", cfg_.speedMode == Logfile::SpeedMode::EPOCH ? modes[0] : modes[1])) {
            if (ImGui::Selectable(modes[0])) {
                cfg_.speedMode = Logfile::SpeedMode::EPOCH;
                cfg_.noLimit = false;
                logfile.SetSpeed(cfg_.speedMode, cfg_.speedValue);
            }
            if (ImGui::Selectable(modes[1])) {
                cfg_.speedMode = Logfile::SpeedMode::MSG;
                cfg_.noLimit = false;
                logfile.SetSpeed(cfg_.speedMode, cfg_.speedValue);
            }
            ImGui::EndCombo();
