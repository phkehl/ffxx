...
void GuiApp::Loop(const Time& now)
{
    ...
    if ((perfStatsCnt_++ % 10) == 0) {
        perfStats_.Update();
    }
    if ((threadsInfoCnt_++ % 30) == 0) {
        threadsInfo_.Update();
    }
...
void GuiApp::DrawFrame()
{
    ...
    perfData_[Perf_e::CPU].Add(perfStats_.cpu_curr_);
    perfData_[Perf_e::RSS].Add(perfStats_.mem_curr_);
...
void GuiApp::_MainMenu()
{
    if (ImGui::BeginMainMenuBar()) {
        ...
        snprintf(text, sizeof(text),
            ...
            ImGui::GetTime(), ImGui::GetFrameCount(), perfStats_.mem_curr_, perfStats_.cpu_curr_);
...
void GuiApp::_DrawDebugWin()
{
    ...
    if (isOpen) {
        if (debugTabbar_.Begin()) {
            debugTabbar_.Item("Log", [this](const GuiWidgetTabbar::State&) { debugLog_.DrawWidget(); });

            debugTabbar_.Item("Tools", [this](const GuiWidgetTabbar::State&) {
            ...
            debugTabbar_.Item("Perf", [this](const GuiWidgetTabbar::State&) {
                ...
                if (ImPlot::BeginPlot("##Performance", plotSize1,
                    ...
                    for (std::size_t plotIx = 0; plotIx < NumOf(plots); plotIx++) {
                        ...
                        ImPlot::PlotLineG(
                            ...
                            pd, pd->data.size(), { ImPlotProp_LineWeight, 2.0f });
                    }
                    ImPlot::EndPlot();
                }
                ...
                for (std::size_t plotIx = 0; plotIx < NumOf(plots); plotIx++) {
                    if (ImPlot::BeginPlot(plots[plotIx].title, plotSize2,
                        ...
                        ImPlot::SetupFinish();
                        PerfData& pd = perfData_[plots[plotIx].perf];
                        ImPlot::PlotHistogram(plots[plotIx].label, pd.data.data(), pd.data.size(), ImPlotBin_Sturges,
                            0.8, range, { ImPlotProp_FillColor, ImPlot::GetColormapColor(plotIx) });
                        ImPlot::EndPlot();
                    }
            ...
            debugTabbar_.Item("Threads", [this](const GuiWidgetTabbar::State&) {
                Gui::TextTitle(ThreadsInfo::HEADER);
                for (const auto& [tid, ti] : threadsInfo_.threads_) {
                    ImGui::Selectable(ti.str_.c_str());
                }
            });

            debugTabbar_.Item("Inputs", [this](const GuiWidgetTabbar::State&) {
                for (const auto& [name, input] : Input::GetAll()) {
                    ImGui::Text(
                        "%2" PRIuMAX " %-20s - %s", input.use_count(), input->name_.c_str(), input->DebugStr().c_str());
                }
            });
