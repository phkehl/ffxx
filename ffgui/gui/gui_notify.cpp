...
GuiNotify::Notification::Notification(const enum Type _type, const std::string& _title, const std::string& _content,
    const double _duration) /* clang-format off */ :
    type        { _type },
    name        { "Notification" + std::to_string(++notiNr) },
    title       { _title },
    content     { _content },
    created     { ImGui::GetTime() },
    duration    { std::clamp(_duration, 1.0, 30.0) },
    destroy     { created + duration },
    remaining   { destroy - created },
    hovered     { false }  // clang-format on
{
}
...
/*static*/ std::vector<GuiNotify::Notification> GuiNotify::notifications_ = {};

/*static*/ std::mutex GuiNotify::mutex_;

/*static*/ void GuiNotify::Message(const std::string& _title, const std::string& _content, const double _duration)
{
    std::lock_guard<std::mutex> guard(mutex_);
    notifications_.push_back(Notification(Notification::MESSAGE, _title, _content, _duration));
}

/*static*/ void GuiNotify::Notice(const std::string& _title, const std::string& _content, const double _duration)
{
    std::lock_guard<std::mutex> guard(mutex_);
    notifications_.push_back(Notification(Notification::NOTICE, _title, _content, _duration));
}

/*static*/ void GuiNotify::Warning(const std::string& _title, const std::string& _content, const double _duration)
{
    std::lock_guard<std::mutex> guard(mutex_);
    notifications_.push_back(Notification(Notification::WARNING, _title, _content, _duration));
}

/*static*/ void GuiNotify::Error(const std::string& _title, const std::string& _content, const double _duration)
{
    std::lock_guard<std::mutex> guard(mutex_);
    notifications_.push_back(Notification(Notification::ERROR, _title, _content, _duration));
}

/*static*/ void GuiNotify::Success(const std::string& _title, const std::string& _content, const double _duration)
{
    std::lock_guard<std::mutex> guard(mutex_);
    notifications_.push_back(Notification(Notification::SUCCESS, _title, _content, _duration));
}
...
void GuiNotify::Draw()
{
    ...
    const Vec2f notiMinSize{ charSize.x * 50, charSize.y };
    const float maxContentHeight = charSize.y * 5;
    const Vec2f notiMaxSize{ charSize.x * 50, maxContentHeight + (4 * charSize.y) };
    Vec2f winPos{ io.DisplaySize.x - charSize.x, io.DisplaySize.y - charSize.y };

    std::lock_guard<std::mutex> guard(mutex_);

    for (auto iter = notifications_.rbegin(); iter != notifications_.rend(); iter++) {
        auto& noti = *iter;
        ...
        const auto winFlags = ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoNav |
        ...
        // Remaining time bar
        if (noti.remaining > 0) {
            const Vec2f bar0 = Vec2f(ImGui::GetCursorScreenPos()) + Vec2f(style.FramePadding.x, -style.FramePadding.y);
            const float width = ImGui::GetContentRegionAvail().x - (2 * style.FramePadding.x);
            const float frac = noti.remaining / noti.duration;
            const Vec2f bar1 = Vec2f(bar0.x + (frac * width), bar0.y);
            auto* draw = ImGui::GetWindowDrawList();
            draw->AddLine(bar0, bar1, C_TEXT_DIM(), 2.0f);
        }
        ...
        noti.hovered = ImGui::IsWindowHovered(
        ...
        if (!noti.content.empty()) {
            ...
            ImVec2 textSize = ImGui::CalcTextSize(
                noti.content.c_str(), nullptr, false, ImGui::GetContentRegionAvail().x - (3 * charSize.x));
            if (ImGui::BeginChild("Content", ImVec2(0, std::min(textSize.y, maxContentHeight)))) {
