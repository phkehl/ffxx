...
//         ImGui::PushStyleVar(ImGuiStyleVar_Alpha, GUI_VAR.imguiStyle->Alpha * 0.65f);
//     }
// }

// void EndDisabled(const bool disabled)
// {
//     if (disabled)
//     {
//         ImGui::PopItemFlag();
...
ImU32 ColourHSV(const float h, const float s, const float v, const float a)
{
    ImColor c;
...
// void SetWindowScale(const float scale)
// {
...
void VerticalSeparator(const float offset_from_start_x)
{
...
void DebugTooltipThingy(const char* fmt, ...)
{
...
bool CheckBoxTristate(const char* label, Tri_e* state, const bool cycleAllThree)
{
    bool res = false;
    switch (*state) {
        case TRI_IGNORE: {
...
{
    //ImGui::PushItemFlag(ImGuiSelectableFlags_NoAutoClosePopups, true);
...
bool ToggleButton(const char* labelOn, const char* labelOff, bool* toggle, const char* tooltipOn,
    const char* tooltipOff, const ImVec2& size)
{
    bool res = false;
    ...
    if (enabled) {
        if (tooltipOn != NULL) {
            ItemTooltip(tooltipOn);
        }
    } else {
        if (tooltipOff != NULL) {
            ItemTooltip(tooltipOff);
...
{
    ImGui::PushStyleColor(ImGuiCol_Text, C_TEXT_TITLE());
    ImGui::TextUnformatted(text);
    ImGui::PopStyleColor();
}
...
void TextTitleF(const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    ImGui::PushStyleColor(ImGuiCol_Text, C_TEXT_TITLE());
...
void SeparatorTitle(const char* text)
{
    ImGui::PushStyleColor(ImGuiCol_Text, C_TEXT_TITLE());
...
{
    if (ImGui::Button(ICON_FK_QUESTION_CIRCLE_O "##HelpButton", GUI_VAR.iconSize)) {
        ImGui::OpenPopup("HelpButton");
    }
    if (ImGui::BeginPopup("HelpButton")) {
