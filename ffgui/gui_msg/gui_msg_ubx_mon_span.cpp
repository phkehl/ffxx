...
void GuiMsgUbxMonSpan::_DrawSpect(const SpectData &spect, const ImVec2 size)
{
    if (ImPlot::BeginSubplots(spect.title_.c_str(), 2, 1, size, ImPlotSubplotFlags_LinkAllX)) {
        ...
        if (ImPlot::BeginPlot("##Spectrum")) {
            ...
            if (resetPlotRange_) {
                ImPlot::SetupAxisLimits(ImAxis_X1, -0.5 * spect.span_, 0.5 * spect.span_, ImPlotCond_Always);
                ImPlot::SetupAxisLimits(ImAxis_Y1, 1.0, 99.0, ImPlotCond_Always);
            }
            ImPlot::SetupFinish();
        ...
        if (ImPlot::BeginPlot("##Heatmap", {-1, 0}, ImPlotFlags_NoLegend)) {
            ...
            if (resetPlotRange_) {
                ImPlot::SetupAxesLimits(-0.5 * spect.span_, 0.5 * spect.span_, NUM_HEAT, 0, ImPlotCond_Always);
            }
            ImPlot::SetupFinish();
