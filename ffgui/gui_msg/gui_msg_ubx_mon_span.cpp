...
void GuiMsgUbxMonSpan::Update(const InputDataMsgPtr& msg)
{
...
void GuiMsgUbxMonSpan::DrawMsg()
{
    ...
    if (tabbar_.Begin()) {
        ...
        for (std::size_t spectIx = 0; spectIx < numSpects_; spectIx++) {
            if (tabbar_.Item(spects_[spectIx].tab_).isSelected_) {
                spectIx0 = spectIx;
                spectIx1 = spectIx;
            }
        }
    ...
    if (ImPlot::BeginSubplots(spect.title_.c_str(), 2, 1, size, ImPlotSubplotFlags_LinkAllX)) {
        ...
        if (ImPlot::BeginPlot("##Spectrum")) {
            ...
            for (auto& label : FREQ_LABELS) {
                ...
                if (std::fabs(spect.centre_ - label.freq_) < spect.span_) {
                    ...
                    ImPlot::PlotInfLines(label.id_, &freq, 1, {ImPlotProp_LineColor, ImPlot::GetColormapColor(0)});
                    ImPlot::PlotText(label.title_, freq, limits.Y.Min, offs,
                        { ImPlotProp_Flags, ImPlotTextFlags_Vertical /*| ImPlotTextFlags_AlignLeft*/,
                            ImPlotProp_LineColor, ImPlot::GetColormapColor(0) });
                }
            }

            // Amplitude with min/max
            ImPlot::PlotShaded("##Amplitude", spect.freq_.data(), spect.max_.data(), spect.min_.data(),
                spect.freq_.size(),
                { ImPlotProp_FillColor, ImPlot::GetColormapColor(1, ImPlotColormap_Deep), ImPlotProp_FillAlpha, 0.7f, ImPlotProp_LineWeight, 2.0f });
            ...
            ImPlot::PlotLine("Mean", spect.freq_.data(), spect.mean_.data(), spect.freq_.size(),
                { ImPlotProp_LineColor, ImPlot::GetColormapColor(2, ImPlotColormap_Deep), ImPlotProp_LineWeight, 2.0f });
        ...
        if (ImPlot::BeginPlot("##Heatmap", {-1, 0}, ImPlotFlags_NoLegend)) {
            ...
            ImPlot::PlotHeatmap<float>("ampl", spect.heat_.data(), NUM_HEAT, UBX_MON_SPAN_V0_NUM_BINS, 0.0f, 100.0f,
                nullptr, { -0.5 * spect.span_, NUM_HEAT }, { 0.5 * spect.span_, 0 },
                { ImPlotProp_Flags, ImPlotHeatmapFlags_None });
            ImPlot::PopColormap();
            ImPlot::EndPlot();
        }
        ImPlot::EndSubplots();
