...
{
    mapParams_ = mapParams;
    cfg_.mapName = mapParams_.name;
...
void GuiWidgetMap::_SetPosAndZoom(const Vec2d& pos, const float zoom, const int zLevel, const float snap)
{
    ...
    zLevel_ = std::clamp(zLevel_, mapParams_.zoomMin, mapParams_.zoomMax);

    numTiles_ = 1 << zLevel_;

    // Update center position
    cfg_.centPosLonLat[0] = std::clamp(pos.x, mapParams_.minLon, mapParams_.maxLon);
    cfg_.centPosLonLat[1] = std::clamp(pos.y, mapParams_.minLat, mapParams_.maxLat);
    ...
    tileSize_ = Vec2d(mapParams_.tileSizeX, mapParams_.tileSizeY) * tileScale;
...
bool GuiWidgetMap::BeginDraw()
{
    ...
    if (autoSize_) {
        const int numTilesX = (canvasSize_.x / mapParams_.tileSizeX) + 1;
        const int numTilesY = (canvasSize_.y / mapParams_.tileSizeY) + 1;
...
void GuiWidgetMap::EndDraw()
{
    ...
    // Map control
    {
        ...
        if (!cfg_.hideConfig) {
            ...
            if (ImGui::BeginChild(1234, GUI_VAR.charSize * Vec2f(30, 5.5), false, ImGuiWindowFlags_NoScrollbar)) {
                ...
                {
                    ImGui::PushItemWidth(-1);
                    if (ImGui::BeginCombo("###BaseMap", mapParams_.title.c_str())) {
                        for (const auto& map : MapParams::BUILTIN_MAPS) {
                            ...
                            if ((entry != GuiGlobal::config_.maps.end()) && entry->second) {
                                const bool selected = (map.name == mapParams_.name);
    ...
    // Map links
    {
        ...
        for (const auto& info : mapParams_.mapLinks) {
