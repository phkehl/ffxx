...
void GuiWinMultimap::Loop(const Time& /*now*/)
{
    ...
    for (auto& [name, input] : Input::GetAll()) {
        ...
        if (state.inputUid_ != input->uid_) {
            input->AddObserver(this, [pState = &entry->second](const InputDataPtr& data) {
                if (data->type_ == InputDataType::EVENT) {
                    auto& event = DataPtrToDataEvent(data);
                    if (event.event_ == InputDataEvent::DBUPDATE) {
                        pState->dbDirty_ = true;
                    }
                }
            });
            state.inputUid_ = input->uid_;
        }
            ...
            for (const auto& [ ts, points ] : points_) {
                ...
                for (const auto& [ state, point ] : points) {
                    if (state->cfg_.used) {
                        ...
                        if (pxy.x != 0.0f) {
                            draw->AddLine(pxy, xy, C_PLOT_MAP_BASELINE()); // C_PLOT_FIX_HL_OK for latest point?
                        }
                        pxy = xy;
                    }
                }
