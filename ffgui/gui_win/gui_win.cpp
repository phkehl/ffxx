...
bool GuiWin::_DrawWindowBegin()
{
    ...
    const bool isFocused = WinIsFocused();
    bool doClose  = (isFocused && (ImGui::IsKeyPressed(ImGuiKey_F4, false) || ImGui::IsKeyPressed(ImGuiKey_W, false)) && ImGui::GetIO().KeyCtrl);
    bool doToBack = (isFocused && ImGui::IsKeyPressed(ImGuiKey_B, false) && ImGui::GetIO().KeyCtrl);
    bool doResize = (isFocused && ImGui::IsKeyPressed(ImGuiKey_R, false) && ImGui::GetIO().KeyCtrl);
    ...
    if (ImGui::BeginPopupContextItem()) {
        if (ImGui::MenuItem("To back  (C-b)"))       { doToBack = true; }
        if (ImGui::MenuItem("Resize   (C-r)"))       { doResize = true; }
        if (ImGui::MenuItem("Close    (C-w, C-F4)")) { doClose  = true; }
        ImGui::EndPopup();
    }

    if (doToBack) {  WinToBack(); }
    if (doResize) {  WinResize(); }
    if (doClose)  {  WinClose(); }
