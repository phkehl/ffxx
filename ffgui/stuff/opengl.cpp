...
{
    if (texture_ != 0) {
        // DEBUG("ImageTexture::Destroy() %" PRIuMAX, texture_);
        GLuint tex = texture_;
...
ImageTexture::ImageTexture(ImageTexture&& rhs) : ImageTexture()
{
    width_ = rhs.width_;
    height_ = rhs.height_;
...
ImageTexture::ImageTexture(const std::string& pngPath, const bool genMipmap) : ImageTexture()
{
    int width = 0;
    int height = 0;
    ...
    texture_ = _MakeTex(width, height, data, genMipmap);
    width_ = width;
    height_ = height;

    stbi_image_free(data);

    // DEBUG("ImageTexture() %s %dx%d %" PRIuMAX, pngPath.c_str(), width_, height_, texture_);
}

// ---------------------------------------------------------------------------------------------------------------------

ImageTexture::ImageTexture(const uint8_t* pngData, const int pngSize, const bool genMipmap) : ImageTexture()
{
    int width = 0;
    int height = 0;
    ...
    // DEBUG("ImageTexture() png %dx%d %" PRIuMAX, width_, height_, texture_);
}

// ---------------------------------------------------------------------------------------------------------------------

ImageTexture::ImageTexture(const int width, const int height, const std::vector<uint8_t>& rgbaData,
    const bool genMipmap) /* clang-format off */ :
    width_   { width },
    height_  { height },
    texture_ { _MakeTex(width, height, rgbaData.data(), genMipmap) }  // clang-format on
{
    // DEBUG("ImageTexture() rgba %dx%d %" PRIuMAX, width_, height_, texture_);
}

// ---------------------------------------------------------------------------------------------------------------------

ImageTexture::ImageTexture(
    const int width, const int height, const uint8_t* rgbaData, const bool genMipmap) /* clang-format off */ :
    width_   { width },
    height_  { height },
    texture_ { _MakeTex(width, height, rgbaData, genMipmap) }  // clang-format on
{
}
...
/* static */ TextureId ImageTexture::_MakeTex(
    const int width, const int height, const uint8_t* data, const bool genMipmap)
{
    GLint lastTex;
...
bool FrameBuffer::Begin(const int width, const int height)
{
    ...
    if ((width != width_) || (height != height_)) {
        ...
        // DEBUG("FrameBuffer: %dx%d, fb %u rb %u tex %u", width, height, framebuffer_, renderbuffer_, texture_);
    }
...
{
    return fbok_ ? (void*)(uintptr_t)texture_ : nullptr;
}

/* ****************************************************************************************************************** */

Shader::Shader(const char* vertexCode, const char* fragmentCode, const char* geometryCode) : _shaderProgram{ 0 }
{
    ...
    if ((vertexShader != 0) && (fragmentShader != 0) && (!geometryCode || (geometryShader != 0))) {
        ...
        DEBUG("Shader() vertex %u, fragment %u, geometry %u -> program %u", vertexShader, fragmentShader,
            geometryShader, _shaderProgram);
...
State::State() /* clang-format off */ :
    ...
    blendEqModeAlpha  { GL_FUNC_ADD } // clang-format on
{
}

// ---------------------------------------------------------------------------------------------------------------------

/*static*/ const std::vector<Enum> State::DEPTH_FUNC  // clang-format off
{
    { GL_NEVER,    "GL_NEVER"    },
    { GL_LESS,     "GL_LESS"     },
    { GL_EQUAL,    "GL_EQUAL"    },
...
/*static*/ const std::vector<Enum> State::CULL_FACE  // clang-format off
{
    { GL_FRONT,          "GL_FRONT"          },
    { GL_BACK,           "GL_BACK"           },
...
/*static*/ const std::vector<Enum> State::CULL_FRONTFACE  // clang-format off
{
    { GL_CW,  "GL_CW"  },
    { GL_CCW, "GL_CCW" },
};  // clang-format on

/*static*/ const std::vector<Enum> State::POLYGONMODE_MODE  // clang-format off
{
    { GL_POINT,  "GL_POINT" },
    { GL_LINE,   "GL_LINE"  },
    { GL_FILL,   "GL_FILL"  },
...
/*static*/ const std::vector<Enum> State::BLENDFUNC_FACTOR  // clang-format off
{
    { GL_ZERO,                     "GL_ZERO"                     },
    { GL_ONE,                      "GL_ONE"                      },
...
/*static*/ const std::vector<Enum> State::BLENDEQ_MODE  // clang-format off
{
