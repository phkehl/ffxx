...
#define DATABASE_COLUMNS(_PN_, _P0_, _P1_, _P2_, _PF_) \
    ...
    \
    _P1_(bytes_total,        double,  NAN,   "%.0f",   "Total bytes since last epoch"               ) \
    _P1_(bytes_rate,         float,   NAN,   "%.1f",   "Average bytes/s since last epoch"           ) \
    _P1_(msgs_total,         double,  NAN,   "%.0f",   "Total messages since last epoch"            ) \
    _P1_(msgs_rate,          float,   NAN,   "%.1f",   "Average messages/s since last epoch"        ) \
    \
    /* end*/
...
{
   public:
    Database(const std::string& name, InputColl&& coll);
    ~Database();
    ...
    // Add data
    void AddEpoch(const InputDataEpoch& inEpoch, const bool isRealTime);
