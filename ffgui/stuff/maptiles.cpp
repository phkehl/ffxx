...
{
    numInstances_++;
    DEBUG("MapTiles(%s) #%d", _mapParams.name.c_str(), numInstances_);
...
OpenGL::TextureId MapTiles::GetTileTex(const int tx, const int ty, const int tz)
{
    ...
    if ((llMin.x < _mapParams.maxLon) && (llMax.x > _mapParams.minLon) && (llMax.y > _mapParams.minLat) &&
        (llMin.y < _mapParams.maxLat)) {
        // Create tile
        {
            std::lock_guard<std::mutex> lock(tilesMutex_);
            ...
            if (!requestQueue_.empty()) {
                if ((int)requestQueue_.size() > MAX_TILES_IN_QUEUE) {
                    ...
                    while ((int)requestQueue_.size() > MAX_TILES_IN_QUEUE) {
                        ...
                        tiles_[requestQueue_.front()].state = Tile::FAILED;
                        requestQueue_.pop();
                    }
                }
        ...
        if (coord) {
            ...
            // Download
                ...
                if (_mapParams.subDomains.size() > 0) {
                    const int ix = subDomainIx_++ % _mapParams.subDomains.size();
                    StrReplace(url, "{s}", _mapParams.subDomains[ix]);
                }
