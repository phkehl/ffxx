...
Database::Database(const std::string& name, InputColl&& coll) /* clang-format off */ :
    name_   { name },
    coll_   { coll }  // clang-format on
{
...
void Database::Clear()
{
    ...
    serial_++;
    coll_.CollectData(std::make_shared<InputDataEvent>(InputDataEvent::DBUPDATE));
}
...
void Database::AddEpoch(const InputDataEpoch& inEpoch, const bool isRealTime)
{
    std::lock_guard<std::mutex> lock(mutex_);
    Row row;
    ...
    row.bytes_total = epoch.bytesTotal;
    row.bytes_rate = epoch.bytesRate;
    row.msgs_total = epoch.msgsTotal;
    row.msgs_rate = epoch.msgsRate;
