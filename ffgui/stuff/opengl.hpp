...
class ImageTexture
{
   public:
    ...
    int GetWidth() const;
    int GetHeight() const;
    TextureId GetTextureId() const { return texture_; }

    // Destroy OpenGL image texture, actively or on destruct
    void Destroy();
    ~ImageTexture();

    // Must not copy texture (as that would call the destructor, which would remove the texture from the GPU)
   ...
   private:
    ...
    static TextureId _MakeTex(const int width, const int height, const uint8_t* data, const bool genMipmap);
};
