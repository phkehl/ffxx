...
Receiver::Receiver(const std::string& name, InputColl&& coll) /* clang-format off */ :
    name_     { name },
    coll_     { coll },
    worker_   { name, std::bind(&Receiver::_Worker, this, std::placeholders::_1) }  // clang-format on
{
    DEBUG("Receiver(%s)", name_.c_str());
}
...
bool Receiver::_Worker(Thread& thread)
{
    ...
    stream_->AddStateObserver([&](const StreamState oldState, const StreamState newState, const StreamError error,
        ...
        switch (newState) {  // clang-format off
                case StreamState::CONNECTING: coll_.CollectLog(LoggingLevel::INFO,   logStr); break;
                case StreamState::CLOSED:     /* FALLTHROUGH */
                case StreamState::CONNECTED:  coll_.CollectLog(LoggingLevel::NOTICE, logStr); break;
                case StreamState::ERROR:      coll_.CollectLog(LoggingLevel::ERROR,  logStr); break;
            }  // clang-format on
    ...
    while (!thread.ShouldAbort() && (streamState_ != StreamState::CLOSED)) {
        if (streamState_ == StreamState::CONNECTED) {
            while (stream_->Read(msg) /*&& !thread.ShouldAbort()*/) {
                coll_.CollectMsg(std::make_shared<InputDataMsg>(msg, InputDataMsg::RCVD));
            }
        }
        sem.WaitFor(123);
    }
...
{
    if (Write(msg.data_)) {
        coll_.CollectMsg(std::make_shared<InputDataMsg>(msg, InputDataMsg::SENT));
        return true;
    } else {
        return false;
    }
