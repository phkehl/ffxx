...
#include <csignal>
#ifdef NDEBUG
#  define IM_ASSERT(_expr_)                   do { if (!(_expr_)) { FATAL("Assertion failed: %s", # _expr_); exit(255); } } while (0)
#  define IM_ASSERT_USER_ERROR(_expr_, _msg_) do { } while (0)
#  define IM_ASSERT_USER_ERROR_RET(_expr_, _msg_) do { return; } while (0)
#  define IM_ASSERT_USER_ERROR_RETV(_expr_, _retv_, _msg_) do { return _retv_; } while (0)
#else
#  define IM_ASSERT(_expr_) do { if (!(_expr_)) { ERROR("%s:%d:%s() Assertion failed: %s", __FILE__,  __LINE__, __FUNCTION__, # _expr_ ); raise(SIGTRAP); }  } while (0)
...
#  define IM_ASSERT_USER_ERROR_RET(_expr_, _msg_) do { if (!(_expr_)) { WARNING("%s:%d:%s() Warning: %s", __FILE__,  __LINE__, __FUNCTION__, _msg_); return; } } while (0)
#  define IM_ASSERT_USER_ERROR_RETV(_expr_, _retv_, _msg_) do { if (!(_expr_)) { WARNING("%s:%d:%s() Warning: %s", __FILE__,  __LINE__, __FUNCTION__, _msg_); return _retv_; } } while (0)
#endif
...
#define IM_VEC2_CLASS_EXTRA \
    constexpr ImVec2(const std::array<float, 2>& a) : x(a[0]), y(a[1]) {} \
    constexpr ImVec2(const std::array<double, 2>& a) : x(a[0]), y(a[1]) {} \
    operator std::array<float, 2>() const { return { x, y }; }

#define IM_VEC4_CLASS_EXTRA \
    constexpr ImVec4(const std::array<float, 4>& a) : x(a[0]), y(a[1]), z(a[2]), w(a[3]) {} \
    constexpr ImVec4(const std::array<double, 4>& a) : x(a[0]), y(a[1]), z(a[2]), w(a[3]) {} \
