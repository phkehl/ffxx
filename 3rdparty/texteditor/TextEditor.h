...
public:
	...
	struct DocPos {
		...
		DocPos() = default;
		DocPos(size_t _line, size_t _index) : line(_line), index(_index) {}
	...
	struct DocSelection {
		...
		DocSelection(DocPos _start, DocPos _end) : start(_start), end(_end) {}
		DocPos start;
		DocPos end;
	};
	...
	struct VisPos {
		...
		VisPos() = default;
		VisPos(size_t _row, size_t _column) : row(_row), column(_column) {}
...
protected:
	...
	struct LineSection {
		...
		LineSection(size_t _startIndex, size_t _endIndex, size_t _columns, size_t _indent) :
			startIndex(_startIndex),endIndex(_endIndex), columns(_columns), indent(_indent) {}
	...
	public:
		...
		Cursor(DocPos _start, DocPos _end) : start(_start), end(_end) {}
	...
	struct BracketPair {
		BracketPair(ImWchar _startChar, DocPos _start, ImWchar _endChar, DocPos _end, size_t _level, bool _visible) :
			startChar(_startChar), start(_start), endChar(_endChar), end(_end), level(_level), visible(_visible) {}

		BracketPair(DocPos _start, DocPos _end) : startChar(0), start(_start), endChar(0), end(_end), level(0), visible(false) {}
	...
	struct Fold {
		Fold() = default;
		Fold(size_t _start, size_t _end) : start(_start), end(_end) {}
		size_t start;
		size_t end;
	};
	...
	struct Row {
		Row() = default;
		Row(size_t _line, size_t _section, size_t _columns) : line(_line), section(_section), columns(_columns) {}
		size_t line;
		size_t section;
		size_t columns;
	};
	...
	struct MiniMap {
		...
		// properties
		struct Section {
			Section(size_t _start, size_t _end, Color _color) : start(_start), end(_end), color(_color) {}
			size_t start;
			size_t end;
			Color color;
		};
	...
	struct Marker {
		Marker(ImU32 _lineNumberColor, ImU32 _textColor, const std::string_view& _lineNumberTooltip, const std::string_view& _textTooltip) :
			lineNumberColor(_lineNumberColor), textColor(_textColor), lineNumberTooltip(_lineNumberTooltip), textTooltip(_textTooltip) {}
	...
	// list of squiggles
	struct Squiggle {
		Squiggle(size_t _type, ImU32 _color, const std::string_view& _tooltip) : type(_type), color(_color), tooltip(_tooltip) {}
		size_t type;
		ImU32 color;
		std::string tooltip;
	};
