   ...
   public:
    StreamToolOptions()  // clang-format off
        : ProgramOptions("streamtool", {
            ...
            { 'n', false, "nomsg"  }, { 'N', false, "nostats" }, { 'e', false, "epoch" }, { 'c', false, "compact" } })  // clang-format on
    {
    ...
    bool                 nostats_     = false;
    std::string          spec_;
    bool                 epochDetect_ = false;   //!< Detect and dump epochs
    EpochCollector::Mode epochMode_   = EpochCollector::Mode::RECEIVER;  //!< Epoch detection mode
    bool                 compact_     = false;
    // clang-format on
    ...
    void PrintHelp() override final
    {
        ...
        std::fputs(
            ...
            "    -e, --epoch              -- Detect and dump epochs, multiple -e to change detect/collect method\n"
            "    -c, --compact            -- Compact output (not with -n or -f, implies -N)\n"
            "    <stream>                 -- Stream spec\n"
            "\n", stdout);
    ...
    bool CheckOptions(const std::vector<std::string>& args) final
    {
        ...
        DEBUG("compact   = %s", ToStr(compact_));

        if (spec_.empty()) {
            WARNING("Missing stream spec");
            ok = false;
        }
        if (filter_ && (hexdump_ || !json_.empty() || nomsgs_ || nostats_ || epochDetect_ || compact_)) {
            WARNING("Cannot do '-xjnNec' with '-f'");
            ok = false;
        }
        if (nomsgs_ && compact_) {
            WARNING("Cannot do '-n' with '-c'");
            ok = false;
        }
...
class StreamTool
{
        ...
        // Dump mode
            ...
            while (!sigint.ShouldAbort() && ok && !closed) {
                ...
                if (stream->Read(msg, 50)) {
                    ...
                    if (!opts_.nomsgs_ && msgPartOfEpoch) {
                    ...
                    if (!opts_.nostats_ && ((now - last_stat).GetSec() >= 5.0)) {
   ...
   private:
    ...
    void PrintMessageData(const ParserMsg& msg, const Stats::MsgInfo& info)
    {
        ...
        // clang-format off
        std::printf(opts_.compact_ ?
            "message %8" PRIuMAX " %8" PRIuMAX " %5" PRIuMAX " %-8s %-20s %-30s %6.3f %6.3f %s\n" :
            "| %8" PRIuMAX " | %8" PRIuMAX " | %5" PRIuMAX " | %-8s | %-20s | %-30s | %6.3f | %6.3f | %-100s |\n",
    ...
    void PrintEpochData(const Epoch& epoch, const Stats::MsgInfo& info)
    {
        // clang-format off
        std::printf(opts_.compact_ ?
            "epoch   %8" PRIuMAX "        -     - -        EPOCH                EPOCH                          %6.3f %6.3f %s\n"  :
            "| %8" PRIuMAX " | -        | -      | -       | EPOCH                | EPOCH                          | %6.3f | %6.3f | %-100s |\n",
            epoch.seq,
            !info.interval_.IsZero() ? info.interval_.GetSec() : NAN,
            !info.latency_.IsZero()  ? info.latency_.GetSec() : NAN,
    ...
    void PrintStats(const ParserStats& parser_stats)
    {
        ...
        std::printf(fmt, ProtocolStr(Protocol::SBF),    ps.n_sbf_,    (double)ps.n_sbf_    * p_n, ps.s_sbf_,    (double)ps.s_sbf_    * p_s);
        std::printf(fmt, ProtocolStr(Protocol::QGC),    ps.n_qgc_,    (double)ps.n_qgc_    * p_n, ps.s_qgc_,    (double)ps.s_qgc_    * p_s);
    ...
    bool SaveStats(const StreamPtr& stream)
    {
        ...
        json stats ={
            { "parser", {
                ...
                { ProtocolStr(Protocol::SBF),    { { "n_msgs", ps.n_sbf_    }, { "p_msgs", (double)ps.n_sbf_    * p_n }, { "n_bytes", ps.s_sbf_    }, { "p_bytes", (double)ps.s_sbf_    * p_s }, } },
                { ProtocolStr(Protocol::QGC),    { { "n_msgs", ps.n_qgc_    }, { "p_msgs", (double)ps.n_qgc_    * p_n }, { "n_bytes", ps.s_qgc_    }, { "p_bytes", (double)ps.s_qgc_    * p_s }, } },
