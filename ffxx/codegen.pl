...
(
    ...
    # ------------------------------------------------------------------------------------------------------------------
    {
        type => 'NOPE', scheme => 'nope', label => 'Nope (null) stream',
        spec => '',
        mode => 'RW', conn_to => undef, inact_to => undef, retry_to => undef, hotplug => undef,
        docu => [ #                                                                                   ->|
...
do
{
    ...
    $CODEGEN_SECTIONS{FFXX_STREAM_CREDENTIALS} = [];
    ...
    sub doxy { my ($line) = @_; return $line ? " *     $line\n" : " *\n"; }
    ...
    push(@{$CODEGEN_SECTIONS{FFXX_STREAM_CREDENTIALS}},  map { doxy($_); }     @{$STREAM_DOCU{credentials}});
    push(@{$CODEGEN_SECTIONS{FFXX_STREAM_HELP}},         map { help($_); } "", @{$STREAM_DOCU{credentials}});
