 ...
 *     ntripcli(s)  [<credentials>@]<host>:<port>/<mountpoint>[:<version>]     RW  C=10.0  A=10.0  R=5.0   -
 ...
 *     loop         [<delay>][:<rate>]                                         RW  -       -       -       -
 *     fileout      <file>[:<swap>[:<ts>[:<conflict>]]]                        WO  -       -       -       -
 ...
 *
 *     serial://<device>[:<baudrate>[:<autobaud>[:<mode>[:<flow>]]]]
 *
 ...
 *
 *     ntripcli(s)://[<credentials>@]<host>:<port>/<mountpoint>[:<version>]
 *
 *     <credentials> is <username>:<password>, =<base64_encoded_credentials> or %<file> to read
 *     either from a file (see below), <host> address (<IPv4> or [<IPv6>]) or hostname, <port> port
 *     number <mountpoint> name of the caster mountpoint, <version> NTRIP version 'auto' (default),
 *     'v1' or 'v2'
 *
 ...
 *
 *     telnet(s)://<host>:<port>[:<baudrate>[:<autobaud>[:<mode>[:<flow>]]]]
 *
 ...
 *
 *     ntripsvr(s)://<credentials>@<host>:<port>/<mountpoint>[:<version>]
 *
 ...
 *
 *     spidev://<device>[:<speed>[:<bpw>[:<xfersize>[:<mode>]]]]
 *
 ...
 *
 *     canstr://<dev>:<canid_in>:<canid_out>[:<ff>[:<fd>[:<brs>]]]
 *
 ...
 *
 *     gga://<lat>/<lon>/<height>[[:<interval>]:<talker>]
 *
 ...
 *     <conflict> defines the behaviour when the file already exists: 'rep' (replace existing
 *     file, default), 'app' (append to existing file, not with 'ts'), 'ren' (auto rename <file>)
 *
 * @subsubsection FFXX_STREAM_TYPES_FILEIN File read (RO)
 *
 *     filein://<file>[:<speed>[:<offset>]]
 *
 ...
 *
 * @subsubsection FFXX_STREAM_TYPES_NOPE Nope (null) stream (RW)
 *
 *     nope://
 *
 *     Discards any data written to the it, never reads anything
 ...
 *
 * @subsection FFXX_STREAM_CREDENTIALS Credentials
 *
 * @fp_codegen_begin{FFXX_STREAM_CREDENTIALS}
 *     To load credentials from a file (%<file>) the file must exist in the directory given by the
 *     FFXX_STREAM_CREDENTIALS_PATH environment variable, respectively in the current working directory
 *     if the variable is empty.
 * @fp_codegen_end{FFXX_STREAM_CREDENTIALS}
 *
...
enum class StreamType  // clang-format off
{
    ...
    NOPE,                 //!< Nope (null) stream, see @ref FFXX_STREAM_TYPES_NOPE
...
struct StreamOpts
{
    ...
    std::string                  spec_;                              //!< Canonical spec. This may contain secrets!
    ...
    // Path to load credential files from
    //!< Name of the environment variable to use for the path to directory where credentials can be loaded from, empty = current directory
    static constexpr const char* CREDENTIALS_PATH_ENV = "FFXX_STREAM_CREDENTIALS_PATH";
