...
const char* StreamTypeStr(const StreamType type)
{
    switch (type) {  // clang-format off
        ...
        case StreamType::NOPE:                return "NOPE";
...
const char* StreamHelpScreen()
{
    ...
    "\n"
    "    ntripcli(s)://[<credentials>@]<host>:<port>/<mountpoint>[:<version>] (RW) -- NTRIP client (opt. TLS)\n"
    "\n"
    ...
    "\n"
    "    fileout://<file>[:<swap>[:<ts>[:<conflict>]]] (WO) -- File writer\n"
    "\n"
...
static constexpr std::array<StreamTypeInfo, 23> kStreamTypeInfos = {{
    ...
    { StreamType::NOPE,              "nope",         StreamMode::RW,      -1.0,  -1.0,  -1.0,  -1, },
}};
...
/*static*/ StreamOptsPtr StreamOpts::FromSpec(const std::string& spec, std::string& e_msg)
{
    ...
    //     string::StrJoin(options, "][").c_str());  // leaks secrets!
    ...
    switch (info->type_) {  // clang-format off
        ...
        case StreamType::NOPE:      stream_opts =       StreamOptsNope::FromPath(path, path_errors);              break;
    }  // clang-format on
    if (!stream_opts) {
...
/*static*/ StreamPtr Stream::FromOpts(StreamOptsPtr& opts)
{
    ...
    switch (opts->type_) {  // clang-format off
        ...
        case StreamType::NOPE:      stream = std::make_unique<StreamNope       >( dynamic_cast<const StreamOptsNope&       >(*opts ) ); break;
