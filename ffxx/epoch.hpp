...
struct SatInfo  // clang-format off
{
    ...
    SatOrb      orbAvail        = SatOrb::UNKNOWN;        // Bits of SatOrb
...
struct Epoch  // clang-format off
{
    ...
    PerGnssCnt  numSigUsedL1;
    PerGnssCnt  numSigUsedE6;
    PerGnssCnt  numSigUsedL2;
    PerGnssCnt  numSigUsedL5;
...
{
   public:
    enum class Mode { RECEIVER, CORRECTIONS };
    EpochCollector(const Mode mode);
    ~EpochCollector();

    EpochPtr Collect(const fpsdk::common::parser::ParserMsg& msg, bool* msgPartOfEpoch = nullptr);
   ...
   private:
    ...
    std::unique_ptr<CollPersist> persist_;
    std::unique_ptr<CollState> state_;
};

// ---------------------------------------------------------------------------------------------------------------------

bool IsNmeaTos(const fpsdk::common::parser::ParserMsg& msg);
bool IsUbxTos(const fpsdk::common::parser::ParserMsg& msg);
