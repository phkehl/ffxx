...
StreamTcpsvr::StreamTcpsvr(const StreamOptsTcpsvr& opts) /* clang-format off */ :
    StreamBase(opts),
    opts_     { opts },
    thread_   { opts.name_, std::bind(&StreamTcpsvr::Worker, this) }  // clang-format on
{
    b_opts_ = &opts_;
    SetStateClosed();
...
void StreamTcpsvr::CloseClient(Client* client, const bool remove)
{
    ...
    if (remove) {
        ...
        // Client may have left while write was ongoing and we won't get a chance in OnWrite->DoWrite to check that and
        // reset the tx_ongoing_ flag
        bool all_done = true;
