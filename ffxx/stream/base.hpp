...
#endif
#define STREAM_DEBUG(...)  if (!b_opts_->quiet_) { StreamLoggingPrint(fpsdk::common::logging::LoggingLevel::DEBUG,   0, __VA_ARGS__); }
#define STREAM_INFO(...)   if (!b_opts_->quiet_) { StreamLoggingPrint(fpsdk::common::logging::LoggingLevel::INFO,    0, __VA_ARGS__); }
...
{
   public:
    StreamBase(const StreamOpts& opts_init);
    virtual ~StreamBase();
   ...
   protected:
    const StreamOpts* b_opts_ = nullptr;  // Stream options, see base.cpp for explanation
