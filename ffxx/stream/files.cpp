    ...
    const std::string& path, std::vector<std::string>& errors)
{
    ...
    if ((parts.size() >= 1) && (parts.size() <= 4)) {
        ...
        if (parts.size() >= 4) {
            if (parts[3] == "rep") {
                opts.conflict_ = Conflict::REPLACE;
            } else if (parts[3] == "app") {
                opts.conflict_ = Conflict::APPEND;
            } else if (parts[3] == "ren") {
                opts.conflict_ = Conflict::RENAME;
            } else {
                ok = false;
                errors.push_back("bad <conflict>");
            }
        }
    } else {
        ok = false;
    ...
    const char* conflict = (opts.conflict_ == Conflict::REPLACE ? "rep" :
        (opts.conflict_ == Conflict::APPEND ? "ren" : (opts.conflict_ == Conflict::RENAME ? "ren" : "?")));
    opts.path_ = /* clang-format off */ swap == 0 ?
        string::Sprintf("%s::%s:%s", opts.templ_.c_str(), opts.ts_ ? "ts" : "", conflict) :
        string::Sprintf("%s:%d:%s:%s", opts.templ_.c_str(), swap, opts.ts_ ? "ts" : "", conflict);  // clang-format on
...
StreamFileout::StreamFileout(const StreamOptsFileout& opts) /* clang-format off */ :
    StreamBase(opts),
    opts_     { opts },
    thread_   { opts.name_, std::bind(&StreamFileout::Worker, this) }  // clang-format on
{
    b_opts_ = &opts_;
    SetStateClosed();
...
{
    const auto filetime = MakeFileTime();
    std::string data_fn = MakeFilePath(filetime);
    STREAM_TRACE("OpenFile %s", data_fn.c_str());

    int open_flags = O_CREAT | O_WRONLY;
    switch (opts_.conflict_) {
        case StreamOptsFileout::Conflict::APPEND:
            open_flags |= O_APPEND;
            break;
        case StreamOptsFileout::Conflict::REPLACE:
            open_flags |= O_TRUNC;
            break;
        case StreamOptsFileout::Conflict::RENAME:
            break;
    }

    const auto parent = std::filesystem::path(data_fn).parent_path();
    if (!std::filesystem::is_directory(parent)) {
        std::error_code ec;
        std::filesystem::create_directories(parent, ec);
        if (ec) {
            SetStateError(StreamError::DEVICE_FAIL, "mkdir: " + ec.message());
            return false;
        }
    }

    const int data_fd = open(data_fn.c_str(), open_flags, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
    if (data_fd < 0) {
    ...
    if (opts_.ts_) {
        ...
        const int ts_fd = open(ts_fn.c_str(), open_flags, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
        if (ts_fd < 0) {
            SetStateError(StreamError::DEVICE_FAIL, "open: " + string::StrError(errno));
            return false;
        }
    ...
    SetStateConnected(std::filesystem::path(data_fn_).filename().string());
...
void StreamFileout::CloseFile()
{
    ...
    if ((data_fd_ != NOFD) && (close(data_fd_) != 0)) {
        STREAM_WARNING("close fail: %s", string::StrError(errno).c_str());
    }
    data_fd_ = NOFD;
    data_fn_.clear();

    if ((ts_fd_ != NOFD) && (close(ts_fd_) != 0)) {
        STREAM_WARNING("close fail: %s", string::StrError(errno).c_str());
    }
    ts_fd_ = NOFD;
    ts_fn_.clear();
...
bool StreamFileout::Worker()
{
    ...
    bool write_ts = opts_.ts_;  // write a ts on start if enabled
...
time::Time StreamFileout::MakeFileTime() const
{
    const auto now = time::Time::FromClockRealtime();
    auto filetime = now;
    if (opts_.align_ && !opts_.swap_.IsZero()) {
        const int period = opts_.swap_.GetSec(0);
        const int64_t sec = now.GetPosix();  // ~ UTC
        const int64_t snap = (sec / period) * period;
        filetime.SetPosix(snap);
    }
    STREAM_TRACE("filetime %s swap %s %s -> %s", now.StrUtcTime().c_str(), opts_.swap_.Stringify().c_str(),
        string::ToStr(opts_.align_), filetime.StrUtcTime().c_str());
    return filetime;
}

// ---------------------------------------------------------------------------------------------------------------------

std::string StreamFileout::MakeFilePath(const time::Time& t) const
{
    const auto utc = t.GetUtcTime(0);
    const auto gps = t.GetWnoTow(time::WnoTow::Sys::GPS, 0);
    ...
    string::StrReplace(file_path, "%j", string::Sprintf("%03.0f", std::floor(t.GetDayOfYear())));
    ...
    // meier -> meier.1, meier.2, ...
    // meier.raw -> meier.1.raw, meier.2.raw, ...
    if ((opts_.conflict_ == StreamOptsFileout::Conflict::RENAME) && path::PathExists(file_path)) {
        const auto original = std::filesystem::path(file_path);
        const auto extension = original.extension();
        auto cand = original;
        std::size_t n = 1;
        while (std::filesystem::exists(cand)) {
            cand = original;
            cand.replace_extension(string::Sprintf("%" PRIuMAX, n));
            cand += extension;
            n++;
            if (n > 1000000) { // there's a limit to what we reasonably can do..
                break;
            }
        }
        file_path = cand.string();
    }
...
StreamFilein::StreamFilein(const StreamOptsFilein& opts) /* clang-format off */ :
    StreamBase(opts),
    opts_          { opts },
    thread_        { opts.name_, std::bind(&StreamFilein::Worker, this) }  // clang-format on
{
    b_opts_ = &opts_;
    SetStateClosed();
    STREAM_TRACE_WARNING();
