...
StreamBase::StreamBase(const StreamOpts& opts_init) /* clang-format off */ :
    // Each implementation class (e.g. StreamSerial) uses its own options struct (e.g. StreamOptsSerial) derived from
    // the StreamOpts base struct. The opts_init here is the common part of implementation options struct so that we can
    // use some of the common options here in the base ctor. The implementation ctor will then set the b_opts pointer to
    // the actual implementation options and so that we can use the common options in the base methods. We deliberately
    // do not want to copy the options, but always refer to the latest implementation options because they may change
    // run-time (e.g. the baudrate value may change).
    write_queue_   { std::max(opts_init.w_queue_size_, StreamOpts::W_QUEUE_SIZE_MIN) }  // clang-format on
{
    if (!opts_init.quiet_) {
        INFO("Stream(%s) %s://%s%s", opts_init.name_.c_str(), string::StrToLower(StreamTypeStr(opts_init.type_)).c_str(),
            opts_init.disp_.c_str(), opts_init.opts_.c_str());
    }
}
...
bool StreamBase::Read(parser::ParserMsg& msg, const uint32_t timeout)
{
    ...
    if ((state_ != StreamState::CONNECTED) || (b_opts_->mode_ == StreamMode::WO)) {
...
bool StreamBase::Write(const uint8_t* data, const std::size_t size, const uint32_t timeout)
{
    ...
    if ((state_ != StreamState::CONNECTED) || (b_opts_->mode_ == StreamMode::RO)) {
...
{
    return (b_opts_->mode_ != StreamMode::WO) && (read_sem_.WaitFor(millis) == thread::WaitRes::WOKEN);
}
...
void StreamBase::ProcessRead(const uint8_t* data, const std::size_t size)
{
    ...
    bool have_msg = false;
    {
        ...
        while (read_parser_.Process(msg)) {
            ...
            if (read_queue_.size() < b_opts_->r_queue_size_) {
    ...
    const fpsdk::common::logging::LoggingLevel level, const std::size_t repeat, const char* fmt, ...) const
{
    ...
    const int len = std::snprintf(line, sizeof(line), "Stream(%s, %s, %s, %s, %04" PRIx16 ") ", b_opts_->name_.c_str(),
...
#else
    const int len = std::snprintf(line, sizeof(line), "Stream(%s) ", b_opts_->name_.c_str());
#endif
...
bool CredentialsToAuth(const std::string& credentials, std::string& auth_plain, std::string& auth_base64)
{
    ...
    // - "%<path>"
        ...
        const auto filename = credentials.substr(1);
        std::string full_path;

        const char* credentials_path = std::getenv(StreamOpts::CREDENTIALS_PATH_ENV);
        if ((credentials_path != NULL) && (credentials_path[0] != '\0')) {
            full_path = std::string(credentials_path) + "/";
        }
        full_path += filename;

        // Filename must not contain path (/etc/shadow, /proc/kmem, etc.), must be resonably small
        if (string::StrContains(filename, "/") || (path::FileSize(full_path) > StreamOpts::MAX_PATH_LEN)) {
            return false;
        }

        std::vector<uint8_t> data;
        if (path::FileSlurp(full_path, data) && !data.empty() && (data.size() < StreamOpts::MAX_PATH_LEN)) {
    ...
    DEBUG("ok=%s %s %s", string::ToStr(ok), string::ToStr(!auth_plain.empty()), string::ToStr(!auth_base64.empty()));
