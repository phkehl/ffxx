...
StreamSpidev::StreamSpidev(const StreamOptsSpidev& opts) /* clang-format off */ :
    ...
    opts_          { opts },
    fd_            { -1 },
    thread_        { opts.name_, std::bind(&StreamSpidev::Worker, this) }  // clang-format on
{
    b_opts_ = &opts_;
    SetStateClosed();
...
bool StreamSpidev::Worker()
{
    ...
    while (!thread_.ShouldAbort()) {
        ...
        while (parser_.Process(msg)) {
            // We expect messages from a known protocol (UBX, NMEA, ...). If we see OTHER this may be an data from
            // unknown protocol or the device telling us that it has no more data (all 0xff) or the device is not
            // connected or not sending (all 0x00). Since we pass all read data from the ioctl() below throught the
            // parser we should not get here for legitimate data (message payloads) consisting of lots of (more than
            // xfer_size_) 0x00 or 0xff.
        ...
        std::size_t tx_size = 0;
        {
            ...
            tx_size = std::min<std::size_t>(write_queue_.Used(), opts_.xfer_size_);
            if (tx_size > 0) {
        ...
        if (tx_size > 0) {
            ...
            switch (opts_.bits_per_word_) {
                case 8:
                    break;
                case 16: {
                    uint16_t *p = (uint16_t*)tx_buf_.data();
                    const std::size_t n = tx_size / sizeof(uint16_t);
                    for (std::size_t o = 0; o < n; o++) {
                        boost::endian::native_to_big_inplace(p[o]);
                    }
                    break;
                }
                case 32: {
                    uint32_t *p = (uint32_t*)tx_buf_.data();
                    const std::size_t n = tx_size / sizeof(uint32_t);
                    for (std::size_t o = 0; o < n; o++) {
                        boost::endian::native_to_big_inplace(p[o]);
                    }
                    break;
        ...
        if (ioctl(fd_, SPI_IOC_MESSAGE(1), &spi_xfer) >= 0) {
            ...
            switch (opts_.bits_per_word_) {
                case 8:
                    break;
                case 16: {
                    uint16_t *p = (uint16_t*)rx_buf_.data();
                    const std::size_t n = opts_.xfer_size_ / sizeof(uint16_t);
                    for (std::size_t o = 0; o < n; o++) {
                        boost::endian::native_to_big_inplace(p[o]);
                    }
                    break;
                ...
                case 32: {
                    uint32_t *p = (uint32_t*)rx_buf_.data();
                    const std::size_t n = opts_.xfer_size_ / sizeof(uint32_t);
                    for (std::size_t o = 0; o < n; o++) {
                        boost::endian::native_to_big_inplace(p[o]);
                    }
                    break;
