...
struct StreamOptsFileout : public StreamOpts  // clang-format off
{
    ...
    bool align_ = true;
    bool ts_ = false;
    enum class Conflict { REPLACE, APPEND, RENAME };
    Conflict conflict_ = Conflict::REPLACE;
...
class StreamFileout : public StreamBase
{
   ...
   private:
    ...
    bool OpenFile();
    void CloseFile();
    time::Time MakeFileTime() const;
    std::string MakeFilePath(const time::Time& t) const;
