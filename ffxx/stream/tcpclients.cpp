...
StreamTcpClientsBase::StreamTcpClientsBase(
    const StreamOptsTcpClientsBase& opts_init) /* clang-format off */ :
    StreamBase(opts_init),
    resolver_        { ctx_ },
    timer_           { ctx_ },
    socket_conn_     { false },
    socket_raw_      { ctx_ },
...
{
    return bb_opts_->tls_ ? socket_ssl_->next_layer() : socket_raw_;
}
...
{
    STREAM_TRACE("ConfigSocket tx_buf_size=%" PRIuMAX " rx_buf_size=%" PRIuMAX, bb_opts_->tx_buf_size_, bb_opts_->rx_buf_size_);
    ...
    RawSocket().set_option(
        boost::asio::socket_base::send_buffer_size(std::min(bb_opts_->TX_BUF_SIZE_MIN, bb_opts_->tx_buf_size_)), ec);
    if (ec) {
        STREAM_WARNING("tx_buf_size_ fail: %s", ec.message().c_str());
    }
    RawSocket().set_option(
        boost::asio::socket_base::receive_buffer_size(std::min(bb_opts_->RX_BUF_SIZE_MIN, bb_opts_->rx_buf_size_)), ec);
    if (ec) {
...
void StreamTcpClientsBase::CloseSocket(const bool shutdown)
{
    ...
    // Discard any remaining data in tx_queue
...
{
    STREAM_TRACE("SetupTls tls_files_path=%s", bb_opts_->tls_files_path_.c_str());
    ...
    if (!bb_opts_->tls_files_path_.empty()) {
        if (!path::PathExists(bb_opts_->tls_files_path_) || !path::PathIsReadable(bb_opts_->tls_files_path_)) {
            STREAM_WARNING("Bad %s=%s", StreamOpts::TLS_FILES_PATH_ENV, bb_opts_->tls_files_path_.c_str());
            return false;
        } else if (path::PathIsFile(bb_opts_->tls_files_path_)) {
            boost::system::error_code ec;
            ssl_.load_verify_file(bb_opts_->tls_files_path_.c_str(), ec);
            if (ec) {
                STREAM_WARNING("Failed loading %s: %s", bb_opts_->tls_files_path_.c_str(), ec.message().c_str());
                ok = false;
            }
        } else {
            ssl_.add_verify_path(bb_opts_->tls_files_path_.c_str());
        }
        STREAM_DEBUG("Peer verification enabled (%s)", bb_opts_->tls_files_path_.c_str());
...
void StreamTcpClientsBase::DoRetry()
{
    ...
    timer_.cancel();
    if (bb_opts_->retry_to_.count() > 0) {
        timer_.expires_after(bb_opts_->retry_to_);
...
void StreamTcpClientsBase::DoResolve()
{
    ...
    SetStateConnecting("resolve " + HostPortStr(bb_opts_->host_, bb_opts_->port_, bb_opts_->ipv6_));
    resolver_.async_resolve(bb_opts_->host_, std::to_string(bb_opts_->port_),
    ...
    const boost::system::error_code& ec, boost::asio::ip::tcp::resolver::results_type results)
{
    ...
    // (IPv6) and 127.0.0.1 (IPv4)
        ...
        for (auto& iter : results) {
            STREAM_DEBUG("Resolve %s -> %s %" PRIu16, HostPortStr(bb_opts_->host_, bb_opts_->port_, bb_opts_->ipv6_).c_str(),
...
bool StreamTcpClientsBase::HandleVerifyCertificate(bool preverified, boost::asio::ssl::verify_context& verify)
{
    ...
    const bool verified = boost::asio::ssl::host_name_verification(bb_opts_->host_)(preverified, verify);
#else
    const bool verified = boost::asio::ssl::rfc2818_verification(bb_opts_->host_)(preverified, verify);
#endif
...
{
    STREAM_TRACE("DoConnTimeout %" PRIiMAX, bb_opts_->conn_to_.count());
    if (bb_opts_->conn_to_.count() > 0) {
        timer_.cancel();
        timer_.expires_after(bb_opts_->conn_to_);
...
{
    STREAM_TRACE("DoInactTimeout %" PRIiMAX, bb_opts_->inact_to_.count());
    if (bb_opts_->inact_to_.count() > 0) {
        timer_.cancel();
        timer_.expires_after(bb_opts_->inact_to_);
    ...
    const std::string& path, std::vector<std::string>& errors, const StreamType type)
{
    ...
    if (std::regex_match(path, m, path_re) && (m.size() == 5)) {
        ...
        // DEBUG("credentials [%s] [%s] [%s]", opts.credentials_.c_str(), opts.auth_base64_.c_str(), opts.auth_plain_.c_str());  // Leaks secrets!
...
//
// Note that this is not fully correct (but seems to work fine in practice with many casters). For example, For example
// Content-type: chunked isn't implemented at all. Some of the header fields (Ntrip-STR, Ntrip-Flags, Cache-control,
...
void StreamNtripcli::OnResponse(const boost::system::error_code& ec, std::size_t /*bytes_transferred*/)
{
    ...
    // Receiving response success
        ...
        if (StrStartsWith(response_, "ICY 200 OK") || StrStartsWith(response_, "OK") ||
            ...
            resp_ok = true;
            // @todo Implement support for transfer-encoding (chunked, I guess) once we see this in the wild.
            //       Perhaps boost::beast has some helpful bits for this.
            if (StrContains(StrToLower(response_), "transfer-encoding")) {
                SetStateError(StreamError::BAD_RESPONSE, "unsupported transfer-encoding");
                resp_ok = false;
            }
        } else {
            resp_ok = false;
